...
void debug(uint64_t value) {
    char *msg = "sending super secret to secret operatives\0";
...
int main(){
    time_t seed = (time(NULL) / 60) * 60; // Seed with cosmic timestamp (Dilating time to enforce kernel injection if using side channel attack route)
    srand(seed);  
